...
describe('Распознавание сущностей', () => {
    test('Выбирает одушевлённый синоним', async () => {
        await text('');
        await text('жучок');
        expect(await text('жучок')).toMatch('Жучок 🐜 за жучка');
    });

    test('Имя и фамилия', async () => {
        await text('');
        ...
        expect(await text('фёдор емельяненко')).toMatch(/Фёдор емельяненко за гарри поттера,/i);
        expect(await text('аллу пугачёву')).toMatch(/Алла пугачёва за фёдора емельяненко,/i);
    });

    test('Согласование', async () => {
        await text('');
        expect(await text('брата какашку')).toMatch('Брат какашка за дедку');
        expect(await text('железного человека')).toMatch('Железный человек за брата какашку');
        ...
        expect(await text('летнюю пчелку')).toMatch('Летняя пчёлка 🐝 за летнего зайчика,');
        expect(await text('зверя')).toMatch('Зверь за летнюю пчёлку,');
        expect(await text('бабу ягу')).toMatch(/баба яга 🧙‍ за зверя/i);
    ...
    test('Управление', async () => {
        await text('');
        expect(await text('Дурачок с переулочка')).toMatch(/Дурачок с переулочка за дедку/i);
        expect(await text('Дурачок с переулочка')).toMatch(
            /Дурачок с переулочка за дурачка с переулочка/i
        );
    });

    test('Множ. число', async () => {
        await text('');
        await text('кошек');
        expect(await text('кошек')).toMatch('Мы кошки. Будем помогать. Кошки за кошек');
        expect(await text('котята')).toMatch('Котята за кошек');
        expect(await text('молоток')).toMatch(/звали котята .* позвали/);
    });

    test('Гендер прилагательного приоритетнее', async () => {
        await text('');
        expect(await tts('чёрного ворона')).toMatch(/чёрный ворон.*чёрный ворон за/i);
        expect(await tts('чёрный ворон')).toMatch(/чёрный ворон.*чёрный ворон за/i);
    });

    test('Приоритет гендера винительного падежа', async () => {
        /**
         * "Внучку" может быть и внучкой и внучок
         * Нужно сохранить жен. как у вин. падежа
         */
        await text('');
        await tts('внучку');
        expect(await tts('внучку')).toMatch('внучка за внучку');
    });

    test('Неодущевленные', async () => {
        await text('');
        expect(await tts('чернила')).toMatch(/звал дедка чернила/i);
    });

    test('Не ищет глаголы', async () => {
        /**
         * Парсер исключает глаголы, но при склонении они могут пролезть
         * Так Чернила превращаются в Чернившая
         */
        await text('');
        expect(await tts('чернила')).toMatch(/звал дедка чернила/i);
    });
...
describe('Сценарий', () => {
    test('Классическая история', async () => {
        await text('');
        await text('Бабку');
        await text('Внучку');
        await text('Жучку');
...
describe('Персонажи', () => {
    test('Приоритет у одушевлённых', async () => {
        await text('');
        expect(await text('баба деда дудка')).toMatch(/Баба деда 👵 за дедку/i);
    });

    test('Исправляет повторения', async () => {
        await text('');
        expect(await text('человека зомби зомби')).toMatch('Человек зомби за дедку');
    });

    describe('Жучка', () => {
        test('Исправленный tts', async () => {
            await tts('');
    ...
    describe('Дедка', () => {
        test('Детка -> дедка', async () => {
            await text('');
            expect(await text('детка')).toMatch('Дедка 👴 за дедку');
        });
    });
    ...
    describe('Какашка', () => {
        test('Живая!', async () => {
            await text('');
            expect(await text('какашку')).toMatch('Какашка 💩 за дедку');
            expect(await text('какашку')).toMatch('Какашка 💩 за какашку');
        });
    });

    describe('Хаги ваги', () => {
        test('Правильно склоняется!', async () => {
            await text('');
            expect(await text('хаги ваги')).toMatch('Давай обнимемся.');
            expect(await text('хаги ваги')).toMatch('Хаги ваги 🫂 за хаги ваги');
        });
    });
});
...
test.skip('Любого персонажа вин. падежа предпочитает Имени-Фамилии им. падежа', async () => {
    await tts('');
...
test('Позвали осла', async () => {
    await text('');
    expect(await text('осла')).toMatch('Осёл за дедку');
...
test.skip('Не принимает два существительныз разного пола', async () => {
    await text('');
...
test('Известные персонажи склоняются правильно', async () => {
    ...
    expect(await tts('🐱 Маленького котёнка')).toMatch(/Маленький котёнок за ворону/i);
...
test('Персонажи с озвучкой 1', async () => {
    ...
    // prettier-ignore
    for (const char of [
        'внучка','слон', 'собака', 'ворону', 'сова', 'курочка', 'лисичка', 'волк', 'петушок',
